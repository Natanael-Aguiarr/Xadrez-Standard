...
turnIndicator.textContent = "Jogam as: Brancas";
turnIndicator.style.color = 'Brancas';
turnIndicator.style.textAlign = 'center';
document.body.appendChild(turnIndicator);


let currentPlayer = "Brancas";
let history = [];
...
for (let i = 0; i < 8; i++) {
    board[i] = [];
    for (let j = 0; j < 8; j++) {
        if (i === 1) board[i][j] = new Piece("Pretas", "\u265F", "pawn"); 
        else if (i === 6) board[i][j] = new Piece("Brancas", "\u2659", "pawn"); 
        else if (i < 2) {
            if (j === 0 || j === 7) board[i][j] = new Piece("Pretas", "\u265C", "rook"); 
            else if (j === 1 || j === 6) board[i][j] = new Piece("Pretas", "\u265E", "knight"); 
            else if (j === 2 || j === 5) board[i][j] = new Piece("Pretas", "\u265D", "bishop"); 
            else if (j === 3) board[i][j] = new Piece("Pretas", "\u265B", "queen"); 
            else if (j === 4) board[i][j] = new Piece("Pretas", "\u265A", "king"); 
        } 
        else if (i > 5) {
            if (j === 0 || j === 7) board[i][j] = new Piece("Brancas", "\u2656", "rook");
            else if (j === 1 || j === 6) board[i][j] = new Piece("Brancas", "\u2658", "knight"); 
            else if (j === 2 || j === 5) board[i][j] = new Piece("Brancas", "\u2657", "bishop"); 
            else if (j === 3) board[i][j] = new Piece("Brancas", "\u2655", "queen"); 
            else if (j === 4) board[i][j] = new Piece("Brancas", "\u2654", "king"); 
        } 
...
function updateBoard(board, save) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        for (let j = 0; j < 8; j++) {
            ...
            cell.className = (i + j) % 2 === 0 ? "Brancas" : "Pretas";
...
function movePiece(selectedPiece, targetPiece) {
    ...
    if (move) {
        ...
        pawnToPiece();
        currentPlayer = currentPlayer === "Brancas" ? "Pretas" : "Brancas";
        turnIndicator.textContent = `Jogam as: ${currentPlayer}`;
...
function pawnMove(thisBoard, fRow, fCol, tRow, tCol, player, enPassant, testCheck) {
    const direction = player === "Brancas" ? -1 : 1;
    ...
    else if (((fRow === 3 && player === "Brancas") || (fRow === 4 && player === "Pretas")) && (fCol + 1 === tCol || fCol - 1 === tCol)
...
function castleMove(color, side) {
    ...
    color === "Brancas" ? (wBigCastle = false, wSmallCastle = false) : (bBigCastle = false, bSmallCastle = false);
    if (color === "Brancas" && !wBigCastleAt) { wBigCastleAt = history.length; }
    if (color === "Brancas" && !wSmallCastleAt) { wSmallCastleAt = history.length; }
    if (color === "Pretas" && !bBigCastleAt) { bBigCastleAt = history.length; }
    if (color === "Pretas" && !bSmallCastleAt) { bSmallCastleAt = history.length; }
    currentPlayer = currentPlayer === "Brancas" ? "Pretas" : "Brancas";
    turnIndicator.textContent = `Jogam as: ${currentPlayer}`;
    updateBoard(board, true);
    isTheEnd();
...
function isCheck(board, fRow, fCol, tRow, tCol, testCasteling) {
    ...
    otherPlayer = currentPlayer === "Brancas" ? "Pretas" : "Brancas";
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
    ...
    if (testCasteling) {
        if (currentPlayer === "Brancas" && board[fRow][fCol].type === "rook") {
        ...
        } else if (currentPlayer === "Pretas" && board[fRow][fCol].type === "rook") {
        ...
        if (board[fRow][fCol].type === "king") {
            currentPlayer === "Brancas" ? (wBigCastle = false, wSmallCastle = false) : (bBigCastle = false, bSmallCastle = false);
            if (currentPlayer === "Brancas" && !wBigCastleAt) { wBigCastleAt = history.length; }
            if (currentPlayer === "Brancas" && !wSmallCastleAt) { wSmallCastleAt = history.length; }
            if (currentPlayer === "Pretas" && !bBigCastleAt) { bBigCastleAt = history.length; }
            if (currentPlayer === "Pretas" && !bSmallCastleAt) { bSmallCastleAt = history.length; }
        }
    }
    return false;
}
...
function endModal(result) {
    ...
    if (result === "Draw") {
        ...
        document.querySelector('.message').innerHTML = result + ' ganharam';
    }
...
function pawnToPieceModal() {
    ...
    document.querySelector('.message').innerHTML = 'Seu peão chegou até o final do tabuleiro, escolha uma peça:';
    modal.classList.add('active');
}
...
function pawnToRook() {
    for (let i = 0; i < 8; i++) {
        if (board[0][i].type === "pawn") {
            board[0][i] = new Piece("Brancas", "\u2656", "rook");
        }
        if (board[7][i].type === "pawn") {
            board[7][i] = new Piece("Pretas", "\u265C", "rook");
        }
    }
    updateBoard(board, true);
...
function pawnToKnight() {
    for (let i = 0; i < 8; i++) {
        if (board[0][i].type === "pawn") {
            board[0][i] = new Piece("Brancas", "\u2658", "knight")
        }
        if (board[7][i].type === "pawn") {
            board[7][i] = new Piece("Pretas", "\u265E", "knight");
        }
    }
    updateBoard(board, true);
    modal.classList.remove('active');
...
function pawnToBishop() {
    for (let i = 0; i < 8; i++) {
        if (board[0][i].type === "pawn") {
            board[0][i] = new Piece("Brancas", "\u2657", "bishop");
        }
        if (board[7][i].type === "pawn") {
            board[7][i] = new Piece("Pretas", "\u265D", "bishop");
        }
    }
    updateBoard(board, true);
    modal.classList.remove('active');
...
function pawnToQueen() {
    for (let i = 0; i < 8; i++) {
        if (board[0][i].type === "pawn") {
            board[0][i] = new Piece("Brancas", "\u2655", "queen");
        }
        if (board[7][i].type === "pawn") {
            board[7][i] = new Piece("Pretas", "\u265B", "queen");
        }
    }
    updateBoard(board, true);
    modal.classList.remove('active');
